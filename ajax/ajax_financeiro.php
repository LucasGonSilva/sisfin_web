...
switch ($_GET['acao']) {
    case 'verificaDescricao':
        ...
        if ($descricao == '') {
            $sql = "SELECT count(id) as valor FROM tb_categoria_financeria WHERE descricao = '$descricao';";
            $query = $db->query($sql);
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $sql = "SELECT count(id) as valor FROM tb_categoria_financeria WHERE descricao LIKE '%$descricao%';";
            $query = $db->query($sql);
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
        }
        if ($result[0]['valor'] == '0') {
